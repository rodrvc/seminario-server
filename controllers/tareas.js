...
const addTarea = (req, res) => {
  ...
  const { id, titulo, descripcion, fecha, autor } = req.body;
  tarea.id = id;
  tarea.titulo = titulo;
...
async function putOneTask(req, res) {
  ...
  const { id } = req.params;
  const update = { titulo: titulo, descripcion: descripcion, fecha: fecha };

  // `doc` is the document _after_ `update` was applied because of
  // `returnOriginal: false`
  let taskUpdate = await Tarea.findOneAndUpdate({ _id: id }, update, {
    new: true,
  });
...
async function deleteOneTask(req, res) {
  // res.send({ messaje: "Se ha intentado borrar" });
  const { id } = req.params;
  console.log(id);
  const deletedItem = await Tarea.findByIdAndDelete(id) // This method is the nice method for deleting
